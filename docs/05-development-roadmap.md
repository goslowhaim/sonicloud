...
- 무료/수동 샘플 데이터만으로 핵심 평가 루프를 검증한다.
...
- 공식 API, 무료 export, 수동 업로드를 우선하고 크롤러는 검토 통과 후 붙인다.
...
- Google Play 앱/리뷰 데이터 접근 방식 확정: 공식/비공식/크롤러 후보 모두 검토
...
- Pydantic: 스키마 검증
- cron 또는 단순 CLI runner: 초기 오케스트레이션
- Prefect 또는 Dagster: 복잡도가 실제로 필요해진 뒤 검토
...
2. 공개/무료 소스에서 하네스 샘플 데이터 20-50개 직접 수집
3. 추출기 인터페이스 작성
4. 점수 계산기 작성
5. 한국어 보고서 템플릿 작성
6. 골든셋 평가기 작성
